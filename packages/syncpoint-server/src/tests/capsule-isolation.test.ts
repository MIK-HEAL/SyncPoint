...
describe("Capsule isolation", () => {
  it("capsules are scoped to task+agent pair", async () => {
    ...
    await ctx.rpc("capsule.create", { taskId: t.id, agentId: a1.id, checkpointId: cp1.id, goal: "a1 goal" });
    await ctx.rpc("capsule.create", { taskId: t.id, agentId: a2.id, checkpointId: cp2.id, goal: "a2 goal" });
  ...
  it("list capsules returns all capsules for a task", async () => {
    ...
    await ctx.rpc("capsule.create", { taskId: t.id, agentId: a.id, checkpointId: cp.id, goal: "v1" });
    await ctx.rpc("capsule.create", { taskId: t.id, agentId: a.id, checkpointId: cp.id, goal: "v2" });
