...
describe("resourceLocatorsOverlap", () => {
  ...
  it("no matcher: exact locator match → overlap", () => {
    const a: ResourceRef = { type: "test", locator: "src/auth.ts", metadata: "", scope: "file" as const };
    const b: ResourceRef = { type: "test", locator: "src/auth.ts", metadata: "", scope: "file" as const };
    expect(resourceLocatorsOverlap(a, b)).toBe(true);
  });

  it("no matcher: different locators → no overlap", () => {
    const a: ResourceRef = { type: "test", locator: "src/auth.ts", metadata: "", scope: "file" as const };
    const b: ResourceRef = { type: "test", locator: "src/api.ts", metadata: "", scope: "file" as const };
  ...
  it("file-type locators are normalized before comparison", () => {
    ...
    const a: ResourceRef = { type: "file", locator: "src/auth.ts", metadata: "", scope: "file" as const };
    const b: ResourceRef = { type: "file", locator: "./src/auth.ts", metadata: "", scope: "file" as const };
    expect(resourceLocatorsOverlap(a, b)).toBe(true);
  });
  ...
  it("registered matcher is used for overlap", () => {
    ...
    const b: ResourceRef = { type: "custom", locator: "src/auth.ts", metadata: "", scope: "file" as const };
    expect(resourceLocatorsOverlap(a, b)).toBe(true);
  });
  ...
  it("no conflict for different resources", () => {
    const claims = [
      ...
      makeResourceClaim({
        ...
        resources: [{ type: "file", locator: "src/api.ts", metadata: "", scope: "file" as const }],
      }),
    ];
...
describe("detectResourceClaimConflicts — scope refinement", () => {
  ...
  it("same file different functions → no conflict", () => {
    const claims = [
      makeResourceClaim({
        id: "c1", actorId: "a1", taskId: "t1",
        resources: [{ type: "file", locator: "src/auth.ts", scope: "function", functionName: "login", metadata: "" }],
      }),
      makeResourceClaim({
        id: "c2", actorId: "a2", taskId: "t2",
        resources: [{ type: "file", locator: "src/auth.ts", scope: "function", functionName: "logout", metadata: "" }],
      }),
    ];
    expect(detectResourceClaimConflicts(claims)).toHaveLength(0);
  });
  ...
  it("line_range non-overlapping → no conflict", () => {
    const claims = [
      makeResourceClaim({
        id: "c1", actorId: "a1", taskId: "t1",
        resources: [{ type: "file", locator: "src/auth.ts", scope: "line_range", lineRange: { start: 1, end: 20 }, metadata: "" }],
      }),
      makeResourceClaim({
        id: "c2", actorId: "a2", taskId: "t2",
        resources: [{ type: "file", locator: "src/auth.ts", scope: "line_range", lineRange: { start: 30, end: 50 }, metadata: "" }],
      }),
    ];
    expect(detectResourceClaimConflicts(claims)).toHaveLength(0);
  });
  ...
  it("line_range overlapping → conflict", () => {
    const claims = [
      makeResourceClaim({
        id: "c1", actorId: "a1", taskId: "t1",
        resources: [{ type: "file", locator: "src/auth.ts", scope: "line_range", lineRange: { start: 10, end: 30 }, metadata: "" }],
      }),
      makeResourceClaim({
        id: "c2", actorId: "a2", taskId: "t2",
        resources: [{ type: "file", locator: "src/auth.ts", scope: "line_range", lineRange: { start: 20, end: 40 }, metadata: "" }],
      }),
    ];
  ...
  it("mixed scope (function vs line_range) → conservative conflict", () => {
    const claims = [
      ...
      makeResourceClaim({
        id: "c2", actorId: "a2", taskId: "t2",
        resources: [{ type: "file", locator: "src/auth.ts", scope: "line_range", lineRange: { start: 50, end: 80 }, metadata: "" }],
      }),
    ];
    expect(detectResourceClaimConflicts(claims)).toHaveLength(1);
  });
