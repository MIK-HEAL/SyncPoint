...
A patch proposal is checked, approved, and applied.
...
|---|---|
| File Claim | Agent A and Agent B both claim `src/shared-config.ts` |
| SyncGate | A hard file-conflict gate is created automatically |
| Sync Transaction | Agent A's checkpoint becomes an approval transaction bound to a gate |
| Patch Proposal | Agent B submits a unified diff after ownership transfer |
| Wake | Session and assignment events create visible sync obligations |
| Sync View | Claims, blockers, patches, and wake queue appear in one view |
...
Agent A                         SyncPoint                         Agent B
  ...
  |                                |<-------------------------------| approve transaction
  | release claim                  |                                |
  |------------------------------->| file gate READY_TO_CONTINUE     |
  |                                | tx gate READY_TO_CONTINUE       |
  |                                |                                |
  |                                |<-------------------------------| submit patch proposal
  | review / approve patch         | PatchProposal APPROVED/APPLIED  |
```
...
- **Claim A / Claim B**: overlapping file claim IDs
- **File conflict gate**: auto-created gate from the overlap
...
| File conflict gate | `SYNC_REQUESTED`, reason `file_conflict` |
...
| Blockers | File-conflict gate and checkpoint transaction gate |
| Wake Queue | Sync obligations created from session/assignment events |
| Patches | Empty before Stage 2 |

This is the key moment of the demo: Agent B has work assigned, but SyncPoint truncates continuation because the file boundary is unsafe.

## Stage 2: Resolve And Apply The Patch
...
1. Agent A acknowledges the file conflict gate.
2. Agent B acknowledges the file conflict gate.
3. The file conflict gate is resolved.
...
8. Agent B submits a patch proposal touching `src/shared-config.ts`.
9. Patch checks pass because Agent B now owns the file.
10. Agent A approves the patch.
11. The patch is marked applied.
...
syncpoint sync status --session <sessionId> --agent <agentBId>
syncpoint patch list --session <sessionId>
syncpoint patch status --patch <patchProposalId>
```
...
| File claims | Agent A released; Agent B remains active |
...
There is currently no direct CLI command for file claims. File claims are exposed through the application layer and MCP tool `syncpoint_file_claim`, so this demo script creates claims directly through the same application service that MCP uses.

The manually visible CLI parts are:

```bash
syncpoint sync status --session <sessionId>
syncpoint sync ack --gate <fileConflictGateId> --agent <agentAId>
syncpoint sync ack --gate <fileConflictGateId> --agent <agentBId>
syncpoint sync resolve --gate <fileConflictGateId> --summary "Ownership transferred."
...
| `.tmp/syncpoint-demo-*/agent-b.patch` | Unified diff submitted as PatchProposal |
...
Then Agent B's patch is checked against claims before approval and application.
Sync View shows the state: claims, blockers, patch, and wake obligations.
```
...
The most common cause is that Agent A's claim was not released before submitting Agent B's patch. Re-run Stage 2 from the original Stage 1 project path.
